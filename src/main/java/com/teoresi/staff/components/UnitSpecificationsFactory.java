...
@Component
public class UnitSpecificationsFactory implements SpecificationFactory<Unit> {

    private final Set<String> searchableFields = Set.of("id",
            "trigram",
            "type",
            "status");
    private final Logger logger = LoggerFactory.getLogger(UnitSpecificationsFactory.class);

    @Override
    public Specification<Unit> createSpecification(String fieldName, SimpleFilterOperator operator, String value) {
    ...
    private Specification<Unit> buildFieldSpecification(String fieldName, SimpleFilterOperator operator, String value) {
        switch (operator) {
            case EQUALS:
